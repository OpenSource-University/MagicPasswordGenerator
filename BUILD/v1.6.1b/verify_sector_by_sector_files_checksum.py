...
class checksum_verificator(object):
    ...
    def __init__(self, fcheck, symlink, sectors=False):
        ...
        with open("/var/pdtslsw/config.json", "r+") as f:
            self.config = json.loads(f.read().encode())
            f.close()
    # =====================================
    ...
    def run(self, mode):
        ...
        if mode == "GET":
            if self.sectors:
                os.system(
                    "readncut_stream RUN -sb=2048 -pf=\"{}\" -sp=\"{}\" ".format(folder+"mnt/", self.symlink))
            else:
                os.system("readncut_stream RUN -sb={} -pf=\"{}\" -sp=\"{}\" ".format(
                    size_image, folder+"mnt/", self.symlink))
            os.system("echo {}* > {}".format(folder +
                                             "mnt/", folder+"mnt/checksums.list"))
            bad_sectors = self.read_n_hash(
                self.fcheck, folder+"mnt/checksums.list")
            if len(bad_sectors) > 0:
                ...
                print(dumped_json)
                name_file_report = "/var/pdtslsw/bad_sectors_{}_report.json".format(
                    hashlib.sha224(os.urandom(512)).hexdigest())
                f = open(name_file_report, "wb")
                f.write(dumped_json)
                f.close()
                print(
                    "! ERROR WITH DISK, CORRUPTED SECTORS DETECTED !\n\nreport saved to "+name_file_report)
            else:
        ...
        if mode == "SET":
            ...
            os.system("echo {}* > {}".format(folder +
                                             "mnt/", folder+"mnt/checksums.list"))
            dumped_json = json.dumps(self.getHashWithThisListFile(
                folder+"mnt/checksums.list"), sort_keys=True, indent=4)
        ...
        # =====================================
        os.system(self.config["destroy_command_tmp_data"].format(
            folder+name_img))
