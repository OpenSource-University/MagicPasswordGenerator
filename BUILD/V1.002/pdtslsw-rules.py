    ...
    def read_one_key(self, symlink, size_key, nb_keygenerated_sub_, temporal_point, elongtheread=[0,1]):
        ...
        for j in range(0,nb_keygenerated_sub_):
            for n in range(0,size_key):
                ...
                printProgressBar(j + 1, nb_keygenerated_sub_, prefix='Current Key({}%) - Global Progress:'.format(int(round((n/size_key)*100.0))),
...
class Rules_Dictator(object):
    ...
    def PIN_GENERATOR(self, TADSAD_SEED):
        ...
        random.seed(BIGSEED)
        RANDOM_DATA="".join([random.randint(0,9) for i in range(1,6)])
    ...
    def KEY_SELECTOR(self, TADSAD_SEED):
        ...
        VERIFICATION_CODES = VERIFICATION_CODES.strip()
        random.seed(BIGSEED)
        if self.yesORnoASK("Generate an key? "):
            name = str(b64enc(os.urandom(40).decode()))
            with open(name,"w") as f:
                [f.write(random.randint(0,255)) for i in range(0,4096)]
                f.close()
            print("Key saved into: {}".format(name))
    # =====================================
    ...
    def yesORnoASK(self,ask):
        print("Generate an key?{} (Yes/No)?".format(ask))
        reply=input(":")
    ...
    def TADSAD2SEED_fn(self, password, temporal_point, symlink_path, nb_keygenerated_sub):
        ...
        if reply=="0":
            data_trv = self.TRV1.dump_n_encryptymined_by_password(nb_keygenerated_sub*2, temporal_point, symlink_path, password, size_key=24, sizerepeat=5, elongtheread=[0, 8], caractrepeat="0",
    ...
    def run(self):
        ...
        print(MagicPasswordInput, 
            str(datetimeOfTimeTravel))
        self.show_output()
